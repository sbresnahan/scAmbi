...
Here we provide an integrated approach that (i) estimates per‑gene overdispersion from Alevin bootstraps in a sparse‑aware way, (ii) combines that estimate with moment‑based and annotation‑driven priors, and (iii) applies the result either by **building a corrected assay** (`RNA_corr`) or by **supplying model offsets**. We then evaluate the impact using **BCV** (biological coefficient of variation) analyses at the between‑sample and within‑sample (cell‑to‑cell) levels.

## How this relates to `edgeR::catchSalmon()`

`catchSalmon` (bulk) computes per‑feature technical OD from bootstraps across **samples**, applies EB moderation with a floor at 1, and analyzes **scaled** counts. Our approach is analogous but uses **cells** as replicates, adds a **moment‑based** component and an **annotation prior**, and supports **offset‑based** workflows in addition to a corrected assay.
...
- **Sparse‑aware C++ kernel**: blockwise processing of the (genes × cells×bootstraps) matrix, strict bounds checking, and efficient dgCMatrix traversal.
...
The following R packages are required for full functionality:
...
with \(\phi_g\) the NB-like biological dispersion.

---

# Integrated overdispersion estimator (\(\mathrm{OD}^{\mathrm{int}}\))

We compute three components per transcript and combine them on the log scale.
...
Form a left-diagonal scaling with entries \(1/\mathrm{OD}^{\mathrm{int,final}}_t\):

\[
Z = \mathrm{diag}\left(\frac{1}{\mathrm{OD}^{\mathrm{int,final}}}\right)\,Y,
\]
...
1. Build a genes × samples pseudobulk by summing counts per sample.
2. Option A - **Direct**: run BCV on `RNA_corr` pseudobulk.    Option B - **Offsets**: run BCV on raw pseudobulk but add offsets:
   - `offset_ratio`: from `pb_corr/pb_raw` (gene × sample).
   - `offset_od`: from an OD vector (gene‑only; applied to all samples).
...
**Interpretation.** Post‑correction, BCV typically decreases for ambiguous genes and the abundance‑dependent trend stabilizes.

## 2) Within‑sample (cell‑to‑cell) BCV

1. Randomly partition cells within a sample into \(G\) groups (defaults: \(G=10\), \(\ge\)50 cells each).
2. Build a genes × groups pseudobulk by summing counts per group.
3. Compute BCV per sample. Offsets may be added using any **feature vector** (gene‑only), mirroring the between‑sample options.
4. Summarize across samples and visualize (per‑sample plots and an overall summary).

**Interpretation.** A successful correction reduces within‑sample (cell‑to‑cell) BCV, especially for highly ambiguous genes, without artificially flattening true biological variability.
